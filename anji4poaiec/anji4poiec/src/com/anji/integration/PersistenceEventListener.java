...
/**
 * Writes genetic algorithm data, including chromosomes and run, to persistent
 * storage.
 *
...
public class PersistenceEventListener implements GeneticEventListener {
    ...
    private static final Logger logger = Logger.getLogger(PersistenceEventListener.class.getName());
    /**
    ...
    /**
     * @param event
     * <code>GeneticEvent.GENOTYPE_EVALUATED_EVENT</code> writes chromosomes and
     * updates run;
     * <code>GeneticEvent.GENOTYPE_START_GENETIC_OPERATORS_EVENT</code> loads
     * config;
     * <code>GeneticEvent.GENOTYPE_FINISH_GENETIC_OPERATORS_EVEN</code> stores
     * config
     */
    @Override
    ...
    private void genotypeFinishGeneticOperatorsEvent() {
        try {
            ...
            logger.log(Level.SEVERE, "could not store configuration", e);
        }
    }
    ...
    private void genotypeEvaluatedEvent(Genotype genotype) {
        ...
        if (persistAllChroms || persistLastGen) {
            for (Chromosome chrom : currentGeneration) {
                storeChromosome(chrom);
            }
            ...
            try {
                ...
                logger.log(Level.SEVERE, "PersistenceEventListener: error storing run", e);
            }
            ...
            if (!persistAllChroms) {
                ...
                for (Chromosome chrom : previousGeneration) {
                    try {
                        db.deleteChromosome(chrom.getId().toString());
                    } catch (Exception e) {
                        logger.log(Level.SEVERE, "error storing chromosome " + chrom, e);
                    }
                }
            }
