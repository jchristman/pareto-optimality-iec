...
public class NeatChromosomeUtility {
    ...
    private static final Logger logger = Logger.getLogger(NeatChromosomeUtility.class.getName());
     ...
     */
    public static Collection<ConnectionAllele> extractConnectionAllelesForDestNeurons(Collection<ConnectionAllele> connAlleles, Collection<Long> destNeuronInnovationIds) {
     ...
     */
    private static List<Allele> initAlleles(short numInputs, short numHidden, short numOutputs, NeatConfiguration config, boolean fullyConnected) {
        ...
        if (fullyConnected) {
            ...
            logger.warning("ignoring intial topology hidden neurons, not fully connected");
        }
    ...
    public static Activator generateANN(Chromosome chrom) {
        assert (chrom != null);

        Activator ann = null;
        try {
            ann = new AnjiActivator(new AnjiNetTranscriber().newAnjiNet(chrom), 1);
