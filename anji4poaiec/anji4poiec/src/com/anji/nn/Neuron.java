 ...
 * @author Philip Tucker
 */
private static class NeuronMetaDataPositionComparator implements Comparator<Object> {
...
public double getValue() {
	if ( dirty ) {
		double sum = 0.0f;
		Iterator<Connection> it = incomingConns.iterator();
		while ( it.hasNext() ) {
...
public static void appendToXml( Collection<?> allNeurons, List<?> outputNeurons, StringBuffer result ) {
	SortedMap<Long, List<NeuronMetaData>> layerToCoords = new TreeMap<Long, List<NeuronMetaData>>();
	int maxLayer = 1;

	// iterate through neurons ...
	Collection<?> nonOuputNeurons = new ArrayList<Object>( allNeurons );
	...
	while ( neuronIter.hasNext() ) {
		...
		if ( metaData != null ) {
			...
			List<NeuronMetaData> layer = (List<NeuronMetaData>) layerToCoords.get( layerNum );
			if ( layer == null ) {
	...
	Iterator<Long> layerIter = layerToCoords.keySet().iterator();
...
private static int inputPos( Neuron n ) {
	int numInputs = 0;
	int totalPos = 0;
	Iterator<Connection> connIter = n.getIncomingConns().iterator();
...
private static NeuronMetaData createMetaData( Neuron neuron, Map<Long, NeuronMetaData> allMetaData ) {
	...
	if ( anInputPos > -1 ) {
		...
		Iterator<Connection> iter = neuron.incomingConns.iterator();
		while ( iter.hasNext() ) {
...
public boolean isRecurrent() {
	Iterator<Connection> iter = getIncomingConns().iterator();
	while ( iter.hasNext() ) {
