 ...
 */
public abstract class CrossoverNoveltyReproductionOperator extends NoveltyReproductionOperator {
 ...
 * size and average novelty of that specie. Species containing only 1 chromosome generate
...
final public void reproduce( final Configuration config, final List<Chromosome> parentChroms,
	...
	if ( parentChroms.size() < 2 )
		...
		while ( offspring.size() < targetSize ) {
			...
			// TODO - use speciated novelty here?
			if ( mother.getNoveltyValue() > father.getNoveltyValue() ) {
				dominant = mother;
				recessive = father;
			}
			else {
