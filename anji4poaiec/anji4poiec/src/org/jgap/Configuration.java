...
public class Configuration implements java.io.Serializable {
     ...
     */
    private HashMap<Long,EvaluationFunction> m_bulkObjectiveFunctions = new HashMap<Long,EvaluationFunction>();
    /**
    ...
    /**
     * Stores a vector that points in an n-dimensional space. The vector will be used by the Genotype,
     * Species, and ChromosomeDominanceComparator to sort the Chromosomes by distance to a line that
     * runs along this vector.
     */
    private HashMap<Long, Double> paretoOptimalityPointingVector = new HashMap<Long, Double>();
    private HashMap<Long, Integer> paretoMinValues = new HashMap<Long, Integer>();
    private HashMap<Long, Integer> paretoMaxValues = new HashMap<Long, Integer>();
    private boolean dynamicPOPV = true;
	...
	public synchronized void setFitnessFunction(FitnessFunction a_functionToSet)
    	...
    	throw new InvalidConfigurationException("Cannot set fitness function with multi-objective jgap");
    }
     ...
     */
    public synchronized void addEvaluationFunction(EvaluationFunction a_functionToSet, double popvWeight)
            throws InvalidConfigurationException {
        verifyChangesAllowed();
        ...
        // EDIT: THIS SHOULDN'T MATTER WITH setFitnessFunction disallowed by exception
        ...
        m_bulkObjectiveFunctions.put(a_functionToSet.getEvaluationFunctionUUID(), a_functionToSet);
        paretoOptimalityPointingVector.put(a_functionToSet.getEvaluationFunctionUUID(), popvWeight);
    }
     ...
     */
    public HashMap<Long, EvaluationFunction> getBulkFitnessFunctions() {
        return m_bulkObjectiveFunctions;
    }
    ...
    public synchronized void verifyStateIsValid() throws InvalidConfigurationException {
        ...
        // appropriate values.
        // ------------------------------------------------------------
        if (m_objectiveFunction == null && m_bulkObjectiveFunctions == null) {
            throw new InvalidConfigurationException(
        ...
        Iterator<FitnessReproductionOperator> it = getFitnessReproductionOperators().iterator();
        while (it.hasNext()) {
	...
	/**
	 * @return the paretoOptimalPointingVector
	 */
	public HashMap<Long,Double> getPOPV() {
		return paretoOptimalityPointingVector;
	}
	
	public void adjustPOPV(HashMap<Long, Integer> chosenFitness) {
		if (dynamicPOPV) {
			for (Entry<Long, Integer> entry : chosenFitness.entrySet()) {
				int max = paretoMaxValues.get(entry.getKey());
				int min = paretoMinValues.get(entry.getKey());
				paretoOptimalityPointingVector.put(entry.getKey(), 
						((double) entry.getValue() - min) / ((double) max - min));
			}
		}
	}

    public boolean isDynamicPOPV() {
		return dynamicPOPV;
	}

	public void setDynamicPOPV(boolean dynamicPOPV) {
		this.dynamicPOPV = dynamicPOPV;
	}
	
	public void setMaxValue(Long UUID, int value) {
		if (paretoMaxValues.containsKey(UUID)) {
			if (paretoMaxValues.get(UUID) < value) {
				paretoMaxValues.put(UUID, value);
			}
		} else {
			paretoMaxValues.put(UUID, value);
		}
	...
	public double getMaxValue(Long UUID) {
		if (paretoMaxValues.containsKey(UUID))
			return paretoMaxValues.get(UUID);
		else
			return 0.0;
	}
	
	public void setMinValue(Long UUID, int value) {
		if (paretoMinValues.containsKey(UUID)) {
			if (paretoMinValues.get(UUID) > value) {
				paretoMinValues.put(UUID, value);
			}
		} else {
			paretoMinValues.put(UUID, value);
		}
	...
	public double getMinValue(Long UUID) {
		if (paretoMinValues.containsKey(UUID))
			return paretoMinValues.get(UUID);
		else
