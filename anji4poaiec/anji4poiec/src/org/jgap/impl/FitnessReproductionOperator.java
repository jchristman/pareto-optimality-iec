    ...
    final public void reproduce(final Configuration config, final List<Specie> parentSpecies, List<ChromosomeMaterial> offspring)
        ...
        if (config.isSteadyState()) {
            if (offspring.size() < 1) {
                ...
                if (config.getFitnessSelector() instanceof MultiObjectiveNSSelector) {
                	parentSpecie = parentSpecies.get(0); // TODO: Support speciation with MOEA? 
                } else {
                	parentSpecie = parentSpecies.get(0); // TODO: Support speciation with MOEA? 
					System.out.println("WARNING: Only MultiObjectiveNSSelector is currently supported. Please change config.");
                }
            ...
            Specie specie = parentSpecies.get(0); // TODO: Support speciation with MOEA?
            reproduce(config, specie.getChromosomes(), targetNewOffspringCount, newOffspring);
