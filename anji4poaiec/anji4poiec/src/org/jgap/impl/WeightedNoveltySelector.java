...
public class WeightedNoveltySelector extends NoveltySelector {
    ...
    protected synchronized List<Chromosome> select(Configuration a_activeConfiguration, int a_howManyToSelect) {
        ...
        if (m_wheel.isEmpty() == false) {
            ...
            // -------------------------------------------------------------------
            Set<Entry<Chromosome,SlotCounter>> entries = m_wheel.entrySet();
            ...
            Iterator<Entry<Chromosome,SlotCounter>> entryIterator = entries.iterator();
            for (int i = 0; i < numberOfEntries; i++) {
                Entry<Chromosome,SlotCounter> chromosomeEntry = entryIterator.next();
